...
const RECIPE_SYSTEM_PROMPT = (storeDataText: string) => `당신은 한국 음식점의 레시피(메뉴 레시피 + 자체소스 레시피)를 DB에 입력하는 어시스턴트입니다.
...
2. 자체소스 레시피 입력/수정 (소스에 들어가는 재료와 용량)
3. 메뉴 상품코드 설정/변경
...
- 자체소스 레시피: 소스명은 제품 목록에 등록된 제품이어야 합니다 (보통 "자체소스" 카테고리)
- @ 기호는 메뉴 구분자입니다. menuName/sauceName/productName에 절대 @ 기호를 포함하지 마세요. (예: "@잭콕" → menuName은 "잭콕")
...
2. 재고 잔량 수정 (inventory_snapshots 테이블):
{
  "description": "변경 내용 설명 (한국어)",
  "actions": [
    {
      "type": "update_inventory",
      "params": { "productName": "제품명", "remaining": 200 }
    }
  ]
}
- remaining(잔량, g 단위)만 사용합니다. quantity는 사용하지 않습니다.
...
export async function POST(request: Request) {
  try {
    ...
    if (!message || typeof message !== "string") {
      return Response.json({ error: "메시지를 입력해주세요.", errorType: "parse_failed" }, { status: 400 });
    }

    if (message.length > 500) {
      return Response.json({ error: "메시지는 500자 이내로 입력해주세요.", errorType: "parse_failed" }, { status: 400 });
    }

    if (!storeId || !userId) {
      return Response.json({ error: "인증 정보가 없습니다.", errorType: "permission_denied" }, { status: 401 });
    }

    if (!checkRateLimit(userId)) {
      return Response.json(
        { error: `일일 요청 한도(${DAILY_LIMIT}회)를 초과했습니다. 내일 다시 시도해주세요.`, errorType: "rate_limited" },
        { status: 429 }
      );
    }
    ...
    if (errorMessage.includes("API key")) {
      return Response.json({ error: "OpenAI API 키가 설정되지 않았습니다.", errorType: "network_error" }, { status: 500 });
    }

    return Response.json({ error: "서버 오류가 발생했습니다.", errorType: "network_error" }, { status: 500 });
  }
}
